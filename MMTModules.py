...
def calculate_crosstalk(det_dict, coupling_dict, freq1, freq2, pixel_size, perc_corr, N, beam_fwhm, sky_decomp, TtoP_suppress, delta_ell, ell_max, choose_normalization, make_plot=False):
    '''
    ...
    ****************************************************************************
    ...
    freq1: band frequency 1 being considered for the analysis
    freq2: band frequency 2 being considered for the analysis
    ...
    ****************************************************************************
    ...
    if make_plot==True:
        ...
        #II
        ax[0,0].imshow(convolved_coupled_beams['II']/np.max(abs(convolved_coupled_beams['II'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #IQ
        ax[0,1].imshow(convolved_coupled_beams['IQ']/np.max(abs(convolved_coupled_beams['IQ'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #IU
        ax[0,2].imshow(convolved_coupled_beams['IU']/np.max(abs(convolved_coupled_beams['IU'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #QI
        ax[1,0].imshow(convolved_coupled_beams['QI']/np.max(abs(convolved_coupled_beams['QI'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #QQ
        ax[1,1].imshow(convolved_coupled_beams['QQ']/np.max(abs(convolved_coupled_beams['QQ'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #QU
        ax[1,2].imshow(convolved_coupled_beams['QU']/np.max(abs(convolved_coupled_beams['QU'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #UI
        ax[2,0].imshow(convolved_coupled_beams['UI']/np.max(abs(convolved_coupled_beams['UI'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        #UQ
        ax[2,1].imshow(convolved_coupled_beams['UQ']/np.max(abs(convolved_coupled_beams['UQ'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        ax[2,2].imshow(convolved_coupled_beams['UU']/np.max(abs(convolved_coupled_beams['UU'])),cmap='RdBu',vmin=vmin,vmax=vmax)
        ...
        fig.colorbar(ax[0,0].imshow(convolved_coupled_beams['II']/np.max(abs(convolved_coupled_beams['II'])),cmap='RdBu',vmin=vmin,vmax=vmax),ax=ax.ravel().tolist())
        plt.show()

    return convolved_coupled_beams

#############################################################################################################################
def calculate_crosstalk2(det_dict, coupling_dict, coupling_dict_2, freq1, freq2, pixel_size, perc_corr, N, beam_fwhm, sky_decomp, TtoP_suppress, delta_ell, ell_max, choose_normalization, plot=False):
    '''
    calculates and returns the 3x3 beam coupling matrix given a crosstalk matrix (coupling_dict) and a detector layout specified in det_dict. Then convolves the pixel beam matrix components with the instrument beam and plots the beam coupling matrix in real space on a log scale.
    ...
    ****************************************************************************
    det_dict [dictionary]: dictionary containing identifying information for the detectors in the focal plane
    coupling_dict [dictionary]: crosstalk matrix defining the detector to detector coupling
    coupling_dict_2 [dictionary]: second crosstalk matrix defining the detector to detector coupling
    freq 1: band frequency 1 being considered for the analysis
    freq 2: band frequency 2 being considered for the analysis
    pixel_size [float]: size of each pixel in arcmin
    perc_corr [float]: percent to which the crosstalk signal is correlated to the carrier signal
    N [int]: number of pixels along a map edge
    beam_fwhm [float]: 3dB point of gaussian beam
    ...
    ****************************************************************************
    ...
    #run a simulation
    beam_matrix = calculate_beam_matrix2(det_dict, coupling_dict, coupling_dict_2, freq1, freq2, pixel_size, perc_corr, N, TtoP_suppress = TtoP_suppress)
    II_total = beam_matrix['II']
    IQ_total = beam_matrix['IQ']
    IU_total = beam_matrix['IU']
    QI_total = beam_matrix['QI']
...
def calculate_beam_matrix(det_dict, coupling_dict, freq1, freq2, pixel_size, perc_corr, N, TtoP_suppress = False, to_fft=False):
    ...
    ****************************************************************************
    ...
    for det in det_dict[freq1].keys():
        ...
        beam_matrix_per_det = crosstalk_pix_map(det, det_dict, coupling_dict, freq1, freq2, perc_corr, beam_map,pixel_size)
        
        II_total += beam_matrix_per_det['II']
        IQ_total += beam_matrix_per_det['IQ']
        IU_total += beam_matrix_per_det['IU']
        QI_total += beam_matrix_per_det['QI']
        QQ_total += beam_matrix_per_det['QQ']
        QU_total += beam_matrix_per_det['QU']
        UI_total += beam_matrix_per_det['UI']
        UQ_total += beam_matrix_per_det['UQ']
        UU_total += beam_matrix_per_det['UU']
    
    #average
    II_total = II_total/num_det
    IQ_total = IQ_total/num_det
    IU_total = IU_total/num_det
    QI_total = QI_total/num_det
    QQ_total = QQ_total/num_det
    QU_total = QU_total/num_det
    UI_total = UI_total/num_det
    UQ_total = UQ_total/num_det
    UU_total = UU_total/num_det 

    beam_keys = ['II','IQ','IU','QI','QQ','QU','UI','UQ','UU']
    beams = [II_total,IQ_total,IU_total,QI_total,QQ_total,QU_total,UI_total,UQ_total,UU_total]
...
#############################################################################################################################
def calculate_beam_matrix2(det_dict, coupling_dict, coupling_dict_2, freq1, freq2, pixel_size, perc_corr, N, TtoP_suppress = False, to_fft=False):
    '''
    calculates the 3x3 beam coupling matrix in IQU space
    ...
    ****************************************************************************
    ...
    for det in det_dict[freq1].keys():
        ...
        beam_matrix_per_det = crosstalk_pix_map2(det, det_dict, coupling_dict, coupling_dict_2, freq1, freq2, perc_corr, beam_map,pixel_size)
...
def crosstalk_pix_map(det, det_dict, coupling_dict, freq1, freq2, perc_corr, empty_beam_map, pixel_size):
    ...
    II = empty_beam_map.copy()
    IQ = empty_beam_map.copy()
    IU = empty_beam_map.copy()
    
    QI = empty_beam_map.copy()
    QQ = empty_beam_map.copy()
    QU = empty_beam_map.copy()
    
    UI = empty_beam_map.copy()
    UQ = empty_beam_map.copy()
    UU = empty_beam_map.copy()
    ...
    if coupling_dict[(freq1,freq2)][det] == 'na':
        ...
        coup_det_ang = coupled_det['ang']
        
        II[i,j] += perc_corr*crosstalk*1.
        IQ[i,j] += perc_corr*crosstalk*math.cos(2.*coup_det_ang)
        IU[i,j] += perc_corr*crosstalk*math.sin(2.*coup_det_ang)
        
        QI[i,j] += 2*perc_corr*crosstalk*math.cos(2.*det_ang)
        QQ[i,j] += 2*perc_corr*crosstalk*math.cos(2.*det_ang)*math.cos(2.*coup_det_ang)
        QU[i,j] += 2*perc_corr*crosstalk*math.cos(2.*det_ang)*math.sin(2.*coup_det_ang)
                                      
        UI[i,j] += 2*perc_corr*crosstalk*math.sin(2.*det_ang)
        UQ[i,j] += 2*perc_corr*crosstalk*math.sin(2.*det_ang)*math.cos(2.*coup_det_ang)
        UU[i,j] += 2*perc_corr*crosstalk*math.sin(2.*det_ang)*math.sin(2.*coup_det_ang)
        
     
    det_ang = det_dict[freq1][det]['ang']

    II[int(N/2),int(N/2)] = det_dict[freq1][det]['sig']*1.
    IQ[int(N/2),int(N/2)] = det_dict[freq1][det]['sig']*math.cos(2.*det_ang)
    IU[int(N/2),int(N/2)] = det_dict[freq1][det]['sig']*math.sin(2.*det_ang)
                                      
    QI[int(N/2),int(N/2)] = 2*det_dict[freq1][det]['sig']*math.cos(2.*det_ang)
    QQ[int(N/2),int(N/2)] = 2*det_dict[freq1][det]['sig']*(math.cos(2.*det_ang)**2)
    QU[int(N/2),int(N/2)] = 2*det_dict[freq1][det]['sig']*math.cos(2.*det_ang)*math.sin(2.*det_ang)
                                      
    UI[int(N/2),int(N/2)] = 2*det_dict[freq1][det]['sig']*math.sin(2.*det_ang)
    UQ[int(N/2),int(N/2)] = 2*det_dict[freq1][det]['sig']*math.sin(2.*det_ang)*math.cos(2.*det_ang)
    UU[int(N/2),int(N/2)] = 2*det_dict[freq1][det]['sig']*(math.sin(2.*det_ang)**2)
    ...
    beam_matrix_per_det = {}
    for i in range(len(beam_keys)):
        beam_matrix_per_det[beam_keys[i]] = beams[i]                                  
    
    return beam_matrix_per_det

#############################################################################################################################
def crosstalk_pix_map2(det, det_dict, coupling_dict, coupling_dict_2, freq1, freq2, perc_corr, empty_beam_map, pixel_size):
    ...
    if coupling_dict[(freq1,freq2)][det] == 'na':
        ...
        UI[i,j] += 2.*perc_corr*crosstalk*math.sin(2.*det_ang)
        UQ[i,j] += 2.*perc_corr*crosstalk*math.cos(2.*coup_det_ang)*math.sin(2.*det_ang)
        UU[i,j] += 2.*perc_corr*crosstalk*math.sin(2.*det_ang)*math.sin(2.*coup_det_ang)
    ...
    UI[int(N/2),int(N/2)] = 2.*det_dict[freq1][det]['sig']*math.sin(2.*det_ang)
    UQ[int(N/2),int(N/2)] = 2.*det_dict[freq1][det]['sig']*math.cos(2.*det_ang)*math.sin(2.*det_ang)
    UU[int(N/2),int(N/2)] = 2.*det_dict[freq1][det]['sig']*(math.sin(2.*det_ang)**2)

    if coupling_dict_2[(freq1,freq2)][det] == 'na':
        crosstalk = 0
        coup_det_ang = 0
        ...
        #take coupled value and calculate crosstalk
        coupled_det = det_dict[freq2][coupling_dict_2[(freq1,freq2)][det]]
        crosstalk =  coupled_det['sig']
